...
var serviceSchema = map[string]any{
	...
	"properties": map[string]any{
		...
		"depends_ready_on": map[string]any{"type": "array", "items": map[string]any{"type": "string"}, "description": "Subset of depends_on that must reach phase ready before this service starts. Every name must also be in depends_on."},
		"hooks": map[string]any{
			"type":                 "object",
			"additionalProperties": false,
			"properties": map[string]any{
				"pre_start":  map[string]any{"type": "array", "items": map[string]any{"type": "string"}, "description": "Argv run after the start claim, before the service spawns. Failing it fails the start."},
				"post_start": map[string]any{"type": "array", "items": map[string]any{"type": "string"}, "description": "Argv run once the service is settled. Failing it is recorded, never fatal."},
				"pre_stop":   map[string]any{"type": "array", "items": map[string]any{"type": "string"}, "description": "Argv run before any signal. Failing it never blocks the stop."},
				"post_stop":  map[string]any{"type": "array", "items": map[string]any{"type": "string"}, "description": "Argv run after the process group is confirmed dead. Never fatal."},
			},
			"description": "Lifecycle hook argvs. Same dir and resolved env as the service; run directly, no shell.",
		},
		"profiles":  map[string]any{"type": "array", "items": map[string]any{"type": "string"}, "description": "Profile names this service belongs to. With --profile or MABO_PROFILE active, a service with a non-empty list runs only when a name overlaps; an absent list always runs."},
		...
		"tty":       map[string]any{"type": "boolean", "description": "Run this service on a pty owned by mabo-ctl's terminal broker and enable `mabo-ctl attach <name>`. Default false: the default /dev/null stdin is what keeps consoles safe."},
